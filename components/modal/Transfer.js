...
import { RolesMap } from '@3rdweb/sdk';
import imageUrlBuilder from '@sanity/image-url';
import { client } from '../../lib/sanity'


const Transfer = ({ selectedToken, thirWebTokens, setAction, walletAddress }) => {
  ...
  useEffect(() => {
    const activeToken = thirWebTokens.find(
      token => token.address === selectedToken.contractAddress
    )
    setActiveThirdWebToken(activeToken);
  }, [thirWebTokens, selectedToken])
  ...
  return (
    <Wrapper>
      <Amount>
        <FlexInputContainer>
          <FlexInput placeholder='0' typeof='number' value={amount}
